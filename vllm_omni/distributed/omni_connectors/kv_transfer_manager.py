...
class OmniKVTransferManager:
        ...
        if self._connector is None:
            ...
            if cfg and (c_type := cfg.get("type")):
                try:
                    ...
                    if c_type == "MooncakeTransferEngineConnector" and c_extra.get("role") == "auto":
    ...
    def _transfer_kv_cache(self, kv_data: KVCacheTransferData, transfer_req_id: str) -> None:
        ...
        #
        # Connectors with supports_raw_data=True (e.g. MooncakeTransferEngineConnector):
        ...
        #
        # Connectors with supports_raw_data=False (e.g. MooncakeStoreConnector/TCP):
            ...
            while True:
                ...
                if result:
                    ...
                    if hasattr(raw_data, "tensor") and hasattr(raw_data, "release"):
                        ...
                        # a non-raw connector (e.g. MooncakeStoreConnector/TCP).
